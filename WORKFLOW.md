...
This document outlines the architecture, multi-board data pipeline, candidate scoring engine, scheduling, and configuration for your self-hosted **Daily AI Job Hunting Agent**. It eliminates the need for paid subscription tools or Apify tokens by running as a **100% free open-source Python engine** locally or in the cloud.
...
graph TD
    ...
    D1[Open-Source JobSpy: LinkedIn, Indeed India, Google Jobs, Glassdoor, ZipRecruiter] -->|2a. Scrape Direct Live Postings| E(Job Aggregator)
    D2[Open Tech APIs: We Work Remotely, RemoteOK, Remotive, Himalayas, Jobicy, Arbeitnow, YC Jobs] -->|2b. Scrape Tech Listings| E
    D3[Communities & ATS: Reddit r/forhire r/jobbit r/remotejobs, Greenhouse, Lever, GitHub] -->|2c. Direct Discovery| E
    ...
    H -->|6a. Google Drive Cloud Upload| I0[Google Drive Folder]
    H -->|6b. Email Dispatch + Drive Link| I1[Brevo / Gmail SMTP]
    H -->|6c. Telegram Mobile Alert + File| I2[Telegram Bot]
    H -->|6d. Ntfy.sh Instant Push + File| I3[Ntfy Mobile App]
    H -->|6e. WhatsApp Alert| I4[Green API / CallMeBot / Twilio]
    H -->|6f. Discord Server Alert| I5[Discord Webhooks]
```
...
### Phase 2: Multi-Source Comprehensive Job Aggregation (100% Open-Source)
- **Primary Engine: Open-Source JobSpy (`python-jobspy`)**:
  - Directly interfaces with live search APIs for **LinkedIn, Indeed India, Glassdoor, and Google Jobs**.
  - Extracts canonical direct apply URLs (`job_url`) and normalized job types (`Full-time`, `Internship`, `Contract`, `Part-time`).
- **Free Open Tech APIs & Developer Communities**:
  - Direct queries to **WeWorkRemotely RSS, RemoteOK, Remotive, Himalayas, Jobicy, Arbeitnow**.
  - Live developer hiring posts from **Reddit (`r/forhire`, `r/jobbit`, `r/remotejobs`)**, **YC Jobs**, and **GitHub tech feeds**.
...
11. **`Source`** — Board origin (*LinkedIn, Indeed, Google Jobs, RemoteOK, etc.*)
12. **`Apply Link`** — Clickable application hyperlink (`Apply Link ↗`)
13. **`Find Referral (LinkedIn)`** — Direct pre-filtered employee search link (`Ask Referral ↗`) to connect with company engineers on LinkedIn.
...
### Phase 6: Google Drive Cloud Upload & Multi-Channel Delivery
- **☁️ Google Drive Upload**: Uploads directly to your designated Google Drive folder and provides a permanent spreadsheet web link.
- **✉️ Email (Brevo / Gmail SMTP)**: HTML digest with attached `.xlsx` spreadsheet + direct Google Drive link.
...
# 2. AI / LLM (Optional - gemini, groq, openai, or blank for keyword engine)
LLM_PROVIDER=gemini
GEMINI_API_KEY=your_gemini_api_key

# 3. EMAIL NOTIFIER (Brevo Free SMTP or Gmail App Password)
EMAIL_ENABLED=true
...
* **SQL Injection Immunity (CWE-89)**: Parameterized queries used across both SQLite and PostgreSQL.
